 ...
 */
export async function decryptFromSender(encrypted, recipientPublicKey, recipientPrivateKey) {
    await init();
    const decrypted = sodium.crypto_box_seal_open(encrypted, recipientPublicKey, recipientPrivateKey);
 ...
 * @returns Encrypted data
 */
export async function encryptAuthenticated(plaintext, nonce, recipientPublicKey, senderPrivateKey) {
    await init();
    const message = typeof plaintext === 'string'
        ? sodium.from_string(plaintext)
        : plaintext;
 ...
 * @returns Decrypted plaintext
 */
export async function decryptAuthenticated(encrypted, nonce, senderPublicKey, recipientPrivateKey) {
    await init();
 ...
 */
export async function encryptKey(symmetricKey, recipientPublicKey) {
 ...
 */
export async function decryptKey(encryptedKey, recipientPublicKey, recipientPrivateKey) {
    const decrypted = await decryptFromSender(encryptedKey, recipientPublicKey, recipientPrivateKey);
