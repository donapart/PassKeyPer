...
import { deriveMasterKey, deriveKeys, generateSalt, generatePassword, calculateStrength, } from '@passkeyper/core';
 ...
 */
ipcMain.handle('calculate-strength', async (_, password) => {
    return calculateStrength(password);
});
/**
 * Create account (derive keys from master password)
 */
ipcMain.handle('create-account', async (_, email, masterPassword) => {
    try {
 ...
 */
ipcMain.handle('login', async (_, email, masterPassword, saltBase64) => {
    try {
 ...
 * List items in vault
 */
ipcMain.handle('list-items', async (_, vaultId) => {
    if (!vaultStorage)
        return [];
 ...
 * Get decrypted item
 */
ipcMain.handle('get-item', async (_, itemId) => {
    if (!vaultStorage)
        throw new Error('Storage not initialized');
 ...
 * Create item
 */
ipcMain.handle('create-item', async (_, vaultId, itemData) => {
    if (!vaultStorage)
        throw new Error('Storage not initialized');
 ...
 * Update item
 */
ipcMain.handle('update-item', async (_, itemId, updates) => {
    if (!vaultStorage)
        throw new Error('Storage not initialized');
 ...
 * Delete item
 */
ipcMain.handle('delete-item', async (_, itemId) => {
    if (!vaultStorage)
        throw new Error('Storage not initialized');
 ...
 * Search items
 */
ipcMain.handle('search-items', async (_, vaultId, query) => {
    if (!vaultStorage)
        return [];
 ...
 * Toggle favorite
 */
ipcMain.handle('toggle-favorite', async (_, itemId) => {
    if (!vaultStorage)
        throw new Error('Storage not initialized');
